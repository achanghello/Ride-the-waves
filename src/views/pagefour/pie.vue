...
export default {
   ...
   data() {
      return {
         datasets: {
            colors: ["purple", "yellow", "pink", "red", "green", "orange"], //颜色
            labels: ["累计确诊", "新增确诊", "累计治愈", "新增治愈", "累计死亡", "累计疑似"], //标签
            values: [130, 60, 100, 20, 10, 0], //值
            x: 170, //圆心x坐标
            y: 110, //圆心y坐标
            radius: 100 //半径
         },
   ...
   watch: {
      showlist: {
         handler(newVal, oldVal) {
           this.datasets.values[0] = Number(newVal.confirmedCount)
           this.datasets.values[1] = Number(newVal.confirmedIncr)
           this.datasets.values[2] = Number(newVal.curedCount)
           this.datasets.values[3] = Number(newVal.curedIncr)
           this.datasets.values[4] = Number(newVal.deadCount)
           this.datasets.values[5] = Number(newVal.suspectedCount)
           this.pieChart(this.pie, this.datasets); //画饼状图
         }
      },
   },
